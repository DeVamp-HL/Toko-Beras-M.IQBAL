...
ok('orang: 10 nama (nota bernama + buku bon termasuk saldo awal + kartu); Bu Rahma 5 kedatangan, selang 2 hari, jam 9 pagi, terakhir 9 Sep → 10 hari tidak datang = BANGKU KOSONG (≥ 2,0× selang); dikenali lewat kartu gaya lama (ciri teks → 4 cip)',
  O.length === 10 && org('bu rahma').kunjungan === 5 && org('bu rahma').selang === 2 && org('bu rahma').jam === 9 && org('bu rahma').waktu === 'pagi' && org('bu rahma').sejak === 10 && org('bu rahma').kosong === true && org('bu rahma').dikenali && org('bu rahma').cip.length === 4 && org('bu rahma').cip[1] === 'kerudung' && org('bu rahma').utang === 37500, JSON.stringify(org('bu rahma')));
ok('orang: Mbak Yuni 6 hari berturut → selang 1, terakhir kemarin → WAKTUNYA DATANG (diharap), bukan kosong; jam 13 siang; belanja 780.000', org('mbak yuni').kunjungan === 6 && org('mbak yuni').selang === 1 && org('mbak yuni').diharap === true && !org('mbak yuni').kosong && org('mbak yuni').jam === 13 && org('mbak yuni').total === 780000, JSON.stringify(org('mbak yuni')));
...
ok('jam dinding: yang dikenali & punya jam duduk di jamnya — Rahma 9, Darto 16, Feri 8, Yuni 13, Ibu Rahma Warung 9 (dua orang di jam 9 = lapis berbeda); 12 angka 06–17 dengan 10 = sekarang; jarum 300°; "sekitar jam 9–11" = dua orang jam 9; sisanya tanpa jam / tak dikenali', JM.orang.length === 5 && JM.orang.find(function (o) { return o.kunci === 'bu rahma'; }).jam === 9 && JM.orang.filter(function (o) { return o.jam === 9; }).map(function (o) { return o.lapis; }).sort().join() === '1,2' && JM.angka.length === 12 && JM.angka.find(function (a) { return a.kini; }).t === 10 && JM.sudut === 300 && JM.sekitar.length === 2 && JM.sekitar[0].kunci === 'bu rahma' && JM.tanpaJam === 5, JSON.stringify([JM.orang.map(function (o) { return o.kunci + '@' + o.jam; }), JM.sudut, JM.sekitar.map(function (o) { return o.kunci; })]));
var MG = susunMinggu(KINI);
ok('minggu: hari ini Sabtu; hanya yang ≥ 3 kedatangan (Darto, Rahma, Yuni); Darto 2 dari 3 di Sabtu → KUAT & paling atas (sel "besar"); Rahma 1 dari 5 → "kecil"; Yuni 0 → nol; 7 orang belum tiga kali datang', MG.hariNama === 'Sabtu' && MG.daftar.length === 3 && MG.daftar[0].kunci === 'pak darto' && MG.daftar[0].kuat && MG.daftar[0].sel[5].kelas === 'besar' && MG.daftar[1].kunci === 'bu rahma' && MG.daftar[1].sel[5].kelas === 'kecil' && MG.daftar.find(function (d) { return d.kunci === 'mbak yuni'; }).sel[5].kelas === 'nol' && MG.kuat.join() === 'Pak Darto' && MG.kurang === 7, JSON.stringify(MG.daftar.map(function (d) { return d.kunci + ':' + d.kali; })));
// ---- BENANG
var BN = susunBenang(KINI, null);
ok('benang: Togar → Feri (3×) tergambar: 2 paku (Togar kiri, Feri kanan), 1 benang; baris menyebut bon Feri; jenis urusan dari aturan', BN.pin.length === 2 && BN.pin[0].kunci === 'bang togar' && BN.pin[0].kiri < 50 && BN.pin[1].kiri > 50 && BN.tali.length === 1 && BN.tali[0].kali === '3×' && /Bang Togar datang untuk Uda Feri/.test(BN.baris[0].teks) && /bon Uda Feri Rp1\.450\.000/.test(BN.baris[0].ket) && BN.jenis.length === 3, JSON.stringify(BN));
...
ok('hafalan: dek = 4 orang bercip, urutan tetap untuk putaran yang sama; kartu pertama punya 3 pilihan (satu benar), pengecoh = yang cirinya paling mirip; belum dijawab → tulisan "siapa ini?"; putaran lain mengacak ulang', HF.dek === 4 && HF.kartu && HF.pilihan.length === 3 && HF.pilihan.filter(function (p) { return p.benar; }).length === 1 && HF.kartu.tulisan === 'siapa ini?' && JSON.stringify(HF.pilihan) === JSON.stringify(HF2.pilihan) && HF.tanpaCiri === 6
...
ok('kartu: ringkasan 5 kali datang · belanja · bon; dikenali → kalimat KR1 "kasir BOLEH"; pola beli terbaca; daftar cip & arah dari aturan', /5 kali datang/.test(KR.ringkas) && /bon Rp37\.500/.test(KR.ringkas) && /BOLEH/.test(KR.bonTeks) && KR.ciriDaftar.length === 16 && /Angsa literan/.test(KR.polaTeks), JSON.stringify(KR.ringkas));
...
ok('atur: bangku kosong 3,0× & nota besar 1.000.000 tersimpan (id tetap "pelanggan") dan BERLAKU: Bu Rahma tidak lagi kosong (10 hari < 3 × 2 = 6? tidak — 10 ≥ 6 masih kosong), nota g1 tetap besar; Yuni tetap diharap; angka mustahil ditolak; menghapus cip yang masih dipakai ditolak',
  !AT.tolak && AT.dokumen[0].data.id === 'pelanggan' && AT.dokumen[0].data.kosongKali === 30 && AT.dokumen[0].data.notaBesar === 1000000 && AT.dokumen[0].data.ciriDaftar.length === 16 && /kosongKali|10/.test(susunAturPelanggan({ kosongKali: '5' }, W).tolak || 'x') && /kerudung/.test(susunAturPelanggan({ ciriDaftar: ATUR_PELANGGAN_BAWAAN.ciriDaftar.filter(function (c) { return c.nama !== 'kerudung'; }) }, W).tolak || '')
  && (function () { var s = simpan(); terapkan(susunAturPelanggan({ kosongKali: '60' }, W)); var r = !semuaOrang(KINI).find(function (b) { return b.kunci === 'bu rahma'; }).kosong && aturPelanggan().kosongKali === 60 && aturPelanggan().dariOwner; pulih(s); return r; })(), JSON.stringify([AT.tolak, susunAturPelanggan({ kosongKali: '5' }, W).tolak]));
// ---- THR
...
ok('bon: rincian FIFO — Feri membayar 110.000 → bon 28 Agu sisa 670.000, bon 5 Sep utuh 780.000; sisa & umur = mesin beku', bon('uda feri').buka.length === 2 && bon('uda feri').buka[0].sisa === 670000 && bon('uda feri').buka[1].sisa === 780000 && bon('uda feri').sisa === 1450000 && bon('uda feri').umur === 22 && SB.reduce(function (a, b) { return a + b.sisa; }, 0) === hitungPiutang().reduce(function (a, b) { return a + b.sisa; }, 0), JSON.stringify(bon('uda feri').buka));
var BB = susunBon(KINI, null, '');
ok('bon: total 3.467.500 dari 6 nama; buku bawaan = sisa terbesar (Feri) dengan halaman 2 bon + 1 pembayaran urut tanggal; papan: Tagih (Darto, Feri, Lisna, Bimo) · Tunggu (Rahma) · Macet (Hartawan); ember: Rahma/Darto/Feri/Bimo 8–30 hari, Lisna & Hartawan > 3 bulan',
  BB.total === 3467500 && BB.berutang === 6 && BB.buku.kunci === 'uda feri' && BB.buku.halaman.length === 3 && BB.buku.halaman[2].jenis === 'bayar' && BB.buku.halaman[0].jenis === 'bon' && BB.papan.filter(function (p) { return p.lajur; }).length === 3 && BB.papan[0].n === 4 && BB.papan.find(function (p) { return p.lajur && /Tunggu/.test(p.judul); }).n === 1 && BB.papan.find(function (p) { return p.lajur && /Macet/.test(p.judul); }).n === 1
  && BB.ember[1].n === 4 && BB.ember[3].n === 2 && susunBon(KINI, null, 'e4').perUmur.length === 2 && susunBon(KINI, null, 'e4').perUmur[0].kunci === 'pak hartawan', JSON.stringify([BB.total, BB.buku, BB.papan.map(function (p) { return p.lajur ? 'L:' + p.judul + ':' + p.n : p.kunci; }), BB.ember]));
