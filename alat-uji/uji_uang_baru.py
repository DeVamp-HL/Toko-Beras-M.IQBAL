...
// ---- PUTARAN 21 (owner 23 Sep: "tanggal 18 gajian, tanggal 19 lanjut kerja — bagaimana catatnya?"): gaji bulan berjalan yang dibayar TENGAH bulan hanya menutup sampai tanggal bayarnya; karyawan berhenti (Gono pensiun); lembar Karyawan (HR)
var bb9Asli = JSON.parse(JSON.stringify(ambilBiayaBulanan().find(function (b) { return b.bulan === '2026-09'; })));
var bb9Ubah = JSON.parse(JSON.stringify(bb9Asli)); bb9Ubah.rincianGaji.push({ nama: 'Gono', hari: 3, gaji: 210000 }); bb9Ubah.tanggalBayarGaji = Object.assign({}, bb9Ubah.tanggalBayarGaji, { Gono: '2026-09-10' }); terapkanKeCache([{ koleksi: 'biayaBulanan', data: bb9Ubah }]);
var HG = hitungUpah('Gono', KINI, 'semua'); var KG10 = kalenderUpah('Gono', KINI, 14);
ok('P21: gaji Sep Gono dibayar 10 Sep di sistem lama → hari yang belum dibayar mulai 11 Sep (BUKAN 1 Okt); 11–18 Sep = 8 penuh × 70.000 = 560.000; kalender 10 Sep lunas & 11 Sep bisa diketuk; absen 10 Sep ditolak, 11 Sep boleh', HG.mulai === '2026-09-11' && HG.sumberMulai === 'lama' && /dibayar 10 Sep/.test(HG.mulaiTeks) && HG.penuh === 8 && HG.upah === 560000
  && KG10.find(function (k) { return k.iso === '2026-09-10'; }).kelas === 'lunas' && KG10.find(function (k) { return k.iso === '2026-09-11'; }).bisa && /sudah termasuk upah/.test(susunAbsen('Gono', '2026-09-10', 1, W).tolak) && !susunAbsen('Gono', '2026-09-11', 1, W).tolak, J([HG.mulai, HG.mulaiTeks, HG.penuh, HG.upah]));
terapkanKeCache([{ koleksi: 'biayaBulanan', data: bb9Asli }]);
var KY = daftarKaryawan(KINI);
ok('P21 HR: daftar karyawan: 3 aktif (Ben, Gono, Hasan), 0 berhenti; Gono: upah belum dibayar 630.000 & kasbon 150.000; Hasan belum ada catatan', KY.aktif.length === 3 && KY.berhenti.length === 0 && KY.baris.find(function (b) { return b.nama === 'Gono'; }).upahBelum === 630000 && KY.baris.find(function (b) { return b.nama === 'Gono'; }).kasbon === 150000 && /belum ada catatan/.test(KY.baris.find(function (b) { return b.nama === 'Hasan'; }).ket), J(KY.baris.map(function (b) { return [b.nama, b.status, b.upahBelum, b.kasbon, b.ket]; })));
ok('P21 HR: berhenti tanpa tanggal ditolak; tanggal berhenti di masa depan ditolak; masuk sesudah berhenti ditolak; "hitung upah sejak" di masa depan ditolak', /isi tanggal berhentinya/.test(susunKaryawan([{ nama: 'Ben Mohsein' }, { nama: 'Gono', status: 'berhenti' }, { nama: 'Hasan' }], W).tolak) && /belum datang/.test(susunKaryawan([{ nama: 'Gono', status: 'berhenti', berhenti: '2026-09-25' }, { nama: 'Ben Mohsein' }, { nama: 'Hasan' }], W).tolak)
  && /sesudah tanggal berhenti/.test(susunKaryawan([{ nama: 'Gono', status: 'berhenti', berhenti: '2026-09-15', masuk: '2026-09-16' }, { nama: 'Ben Mohsein' }, { nama: 'Hasan' }], W).tolak) && /masa depan/.test(susunKaryawan([{ nama: 'Gono' }, { nama: 'Ben Mohsein' }, { nama: 'Hasan', mulaiHitung: '2026-09-30' }], W).tolak));
tulis(susunKaryawan([{ nama: 'Ben Mohsein', peran: 'tetap' }, { nama: 'Gono', peran: 'giliran', status: 'berhenti', berhenti: '2026-09-15' }, { nama: 'Hasan', peran: 'giliran', masuk: '2026-09-17' }], W));
HG = hitungUpah('Gono', KINI, 'semua'); var HH = hitungUpah('Hasan', KINI, 'semua'); KY = daftarKaryawan(KINI);
ok('P21 HR: Gono berhenti 15 Sep → hari 16–18 tidak dihitung (10–15 = 6 penuh = 420.000), akhir 15 Sep, absen 16 Sep DITOLAK "sudah berhenti"; keluar dari chip aktif tapi tetap di daftar dengan tanda awas (upah & kasbon belum beres); Hasan dihitung sejak masuk 17 Sep; tarif tetap 70.000', HG.akhir === '2026-09-15' && HG.penuh === 6 && HG.upah === 420000 && /sudah berhenti sejak 15 Sep/.test(susunAbsen('Gono', '2026-09-16', 1, W).tolak)
  && semuaUpah(KINI).map(function (o) { return o.nama; }).join() === 'Ben Mohsein,Hasan' && KY.berhenti.length === 1 && KY.berhenti[0].nama === 'Gono' && KY.berhenti[0].awas === true && HH.mulai === '2026-09-17' && HH.sumberMulai === 'masuk' && aturUpah().tarif === 70000, J([HG.akhir, HG.penuh, HG.upah, HH.mulai, HH.sumberMulai, KY.berhenti]));
tulis(susunKaryawan([{ nama: 'Ben Mohsein', peran: 'tetap' }, { nama: 'Gono', peran: 'giliran', status: 'aktif' }, { nama: 'Hasan', peran: 'giliran', mulaiHitung: '2026-09-18' }], W));
ok('P21 HR: Gono diaktifkan lagi → 16–18 dihitung kembali (9 penuh); "hitung upah sejak" 18 Sep mengalahkan tanggal otomatis Hasan (sumber atur); menghapus karyawan yang masih punya upah tetap ditolak', hitungUpah('Gono', KINI, 'semua').penuh === 9 && hitungUpah('Hasan', KINI, 'semua').mulai === '2026-09-18' && hitungUpah('Hasan', KINI, 'semua').sumberMulai === 'atur' && /Gono masih punya upah/.test(susunKaryawan([{ nama: 'Ben Mohsein' }, { nama: 'Hasan' }], W).tolak));
tulis(susunKaryawan([{ nama: 'Ben Mohsein', peran: 'tetap' }, { nama: 'Gono', peran: 'giliran' }, { nama: 'Hasan', peran: 'giliran' }], W));
// ---- PUTARAN 22 (owner 23 Sep): BONUS karyawan — ikut upah (baris gaji + slip) atau berdiri sendiri tanpa hari kerja
var HB0 = hitungUpah('Gono', KINI, 'semua', { ketik: '50.000', alasan: 'Rajin' });
ok('P22 bonus ikut upah: Gono upah 630.000 + bonus 50.000 − potong kasbon 150.000 → diterima 530.000; bonus > 10 jt ditolak; bonus tanpa alasan ditolak', HB0.bonus === 50000 && HB0.upah === 630000 && HB0.potong === 150000 && HB0.diterima === 530000 && /paling banyak/.test(susunBayarUpah('Gono', 'semua', W, { ketik: '20.000.000', alasan: 'Rajin' }).tolak) && /alasan/.test(susunBayarUpah('Gono', 'semua', W, { ketik: '50.000' }).tolak), J([HB0.bonus, HB0.upah, HB0.potong, HB0.diterima]));
var RB = susunBayarUpah('Gono', 'semua', W, { ketik: '50.000', alasan: 'Rajin' });   // disusun, TIDAK ditulis (laci dijaga untuk uji K3–K5 di bawah)
ok('P22 bayar dengan bonus (susunan dokumen): baris gaji Gono Sep = 630.000 + 50.000 = 680.000 dengan kolom bonus & alasanBonus; potongan kasbon 150.000 "Dipotong upah"; slip menyebut "Bonus (Rajin) +Rp50.000" dan DITERIMA 530.000', (function () { if (RB.tolak) return false; var b = RB.dokumen.find(function (d) { return d.koleksi === 'biayaBulanan'; }).data; var r = b.rincianGaji.find(function (x) { return x.orang === 'Gono' && x.bonus; }); var k = RB.dokumen.find(function (d) { return d.koleksi === 'kasbonMutasi'; }).data; var sl = RB.slip;
  return r && r.gaji === 680000 && r.bonus === 50000 && r.alasanBonus === 'Rajin' && k.nominal === 150000 && k.caraBayar === 'Dipotong upah' && sl.bonus === 50000 && /Bonus \(Rajin\)\s+\+Rp50\.000/.test(sl.teks) && sl.diterima === 530000; })(), J([RB.tolak, RB.slip && RB.slip.teks]));
var kasB1 = kasPada(); var BS = tulis(susunBonus('Hasan', { ketik: '25.000', alasan: 'Hari raya' }, W));
ok('P22 bonus tanpa hari kerja (Hasan belum punya hari kerja): baris gaji berkunci "Hasan · bonus · 19 Sep 2026" hari 0 gaji 25.000, slipUpah jenis bonus; kas turun 25.000 dari laci; "sejak terakhir dibayar" TIDAK bergeser (slip bonus bukan slip upah); riwayat menyebut bonus; tanpa alasan / nol ditolak', (function () { var b = BS.dokumen[0].data; var r = b.rincianGaji.find(function (x) { return x.nama === 'Hasan · bonus · 19 Sep 2026'; }); var sl = BS.dokumen[1].data;
  return r && r.hari === 0 && r.gaji === 25000 && r.bonus === 25000 && sl.jenis === 'bonus' && sl.diterima === 25000 && kasPada() === kasB1 - 25000 && hitungUpah('Hasan', KINI, 'semua').mulai === '2026-09-19' && hitungUpah('Hasan', KINI, 'semua').sumberMulai === 'kosong' && riwayatUpah('Hasan')[0].jenis === 'slip' && /bonus 19 Sep/.test(riwayatUpah('Hasan')[0].ket) && /alasan/.test(susunBonus('Hasan', { ketik: '10.000' }, W).tolak) && /Isi bonusnya/.test(susunBonus('Hasan', { ketik: '', alasan: 'Rajin' }, W).tolak); })(), J([BS.dokumen[0].data.rincianGaji.map(function (x) { return x.nama; }), kasPada(), kasB1, hitungUpah('Hasan', KINI, 'semua').mulaiTeks]));
identitas('sesudah bonus tanpa hari kerja');
...
if __name__ == '__main__':
    ...
    if '--kontrol' in sys.argv:
        rusak = {
            ...
            # ---- K2
            '"sejak terakhir dibayar" mengabaikan slip sistem baru': js.replace("if (sampai) M = { mulai: ugTambahHari(sampai, 1), sumber: 'slip',", "if (false) M = { mulai: ugTambahHari(sampai, 1), sumber: 'slip',"),
            ...
            'gaji bulan berjalan yang dibayar tengah bulan menutup SELURUH bulan (hari sesudah tanggal bayar terkunci)': js.replace("const tutup = tgl < akhir ? tgl : akhir; if (tutup > sampai) sampai = tutup;", "const tutup = akhir; if (tutup > sampai) sampai = tutup;"),
            'karyawan yang sudah berhenti tetap dihitung upahnya sesudah tanggal berhenti': js.replace("const akhir = O.berhenti && O.berhenti < iso ? O.berhenti : iso;", "const akhir = iso;"),
            'bonus tidak masuk baris gaji (biaya & kas kehilangan bonus)': js.replace("const gaji = Math.round(p.hari * H.tarif) + bonusIni;", "const gaji = Math.round(p.hari * H.tarif);"),
            'slip bonus menggeser "sejak terakhir dibayar" (hari kerja hilang)': js.replace("ambilSlipUpah().forEach((s) => { if (s.jenis === 'bonus') return; if (upKunci(s.nama) === k", "ambilSlipUpah().forEach((s) => { if (upKunci(s.nama) === k"),
            'berhenti tanpa tanggal diterima': js.replace("if (o.status === 'berhenti' && !o.berhenti) return { tolak: o.nama + ' ditandai berhenti — isi tanggal berhentinya' };", ""),
