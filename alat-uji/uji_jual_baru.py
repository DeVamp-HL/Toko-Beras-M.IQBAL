...
ok('T: jam baris rincian & karcis SISA = jam karcis asli 16:00 (bukan jam merinci ' + W.jam + '); jam merinci ada di dirinciPada', W.jam !== '16:00' && sisaDoc && sisaDoc.data.jam === '16:00' && RR.dokumen.filter(function (d) { return d.koleksi === 'penjualan' && d.data.rinciDari === 'k3'; }).every(function (d) { return d.data.jam === '16:00' && typeof d.data.dirinciPada === 'string' && !isNaN(new Date(d.data.dirinciPada).getTime()); }), JSON.stringify(RR.dokumen.map(function (d) { return d.data.jam; })));
