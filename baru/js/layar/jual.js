...
export function pasangLayarJual(akar, opsi) {
  ...
  const aksi = {
    ...
    kcBatalAlasan: (v) => set({ kcBatal: Object.assign({}, S().kcBatal || {}, { alasan: String(v || '').slice(0, 120) }) }),
    kcBatalSimpan: () => { const b = S().kcBatal; if (!b) return; tulisUmum(KC.susunBatalKarcis(b.id, b.alasan, L.waktuSekarang(S().sekarang || undefined))); },
  ...
  function gambarLembar(s, rak, t, info, muncul) {
    ...
    if (s.lembar === 'karcis') {
      ...
      return h`${L1}<div class="lembar ${muncul}" data-k="lembar-${s.lembar}">
        ...
        ${KDH.length ? h`<div class="label">Kasir darurat hari ini · barang dari tuts bernama — batalkan yang salah ketik</div><div class="kartu daftar-nota">${KDH.map((x) => h`<div class="baris-nota" data-k="kd-${x.id}" style="cursor: default;"><div class="atas"><span><b>${x.teks}</b>${x.nama ? ' · ' + x.nama : ''}</span><span class="n">${RP(x.nominal)}</span></div><div class="ket">${x.jam} · ${x.cara}${x.oleh ? ' · ' + x.oleh : ''}${pilBatal(x.id)}</div></div>${formBatal(x)}`)}</div>` : ''}
        ${R.length ? h`<div class="label">Rincian hari ini · tarik balik kalau keliru</div><div class="kartu daftar-nota">${R.map((r) => h`<div class="baris-nota" data-k="rr-${r.grupNota}" style="cursor: default;"><div class="atas"><span>${r.n} barang dari karcis ${KC.kcEkor(r.asliId)} · ${tanggalPendek(r.tanggal)} ${r.jam}${r.jamRinci ? ' · dirinci ' + r.jamRinci : ''}</span><span class="n">${RP(r.total)}</span></div>${tolakKunciTanggal(r.tanggal, '') ? h`<div class="kaca-btn putus" style="min-height: 32px; font-size: 11.5px; margin-top: 4px;" data-aksi="tunjukNota" data-id="${r.idPertama}">${tolakKunciTanggal(r.tanggal, '').split(' — ')[0]} — buat retur hari ini</div>` : r.utuh ? h`<div class="kaca-btn putus" style="min-height: 32px; font-size: 11.5px; margin-top: 4px;" data-aksi="urungRinci" data-g="${r.grupNota}" data-asli="${r.asliId}">tarik balik rincian ini</div>` : h`<div class="ket">sebagian sudah dikoreksi — tidak bisa ditarik balik utuh</div>`}</div>`)}</div>` : ''}
