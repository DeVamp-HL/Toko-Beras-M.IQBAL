...
export const MN_KERJA = [
  ...
  { id: 'harga', nama: 'Katalog harga', ikon: 'label', ket: 'harga jual disunting', tujuan: { ke: 'harga', keluarga: 'katalog', teks: 'buka Katalog harga' } },
];
...
export function susunLaci(kini, lokal) {
  ...
  return [
    { id: 'buku', nama: 'Buku besar', ket: 'yang dibaca: utang, piutang, laporan', isi: [
      baris('pemasok', 'truk', 'Pemasok & utang', U.n ? U.n + ' pemasok · ' + U.bon.length + ' bon terbuka' + (U.lewat.length ? ' · ' + U.lewat.length + ' lewat tempo' : ' · belum ada yang lewat tempo') + (U.tekor ? ' · kelebihan bayar ' + RP(U.tekor) : '') : 'tidak ada utang ke pemasok yang tercatat', U.total ? RP(U.total) : 'nihil', 'utang toko ke pemasok', U.lewat.length > 0 || U.tekor > 0, { ke: 'harga', keluarga: 'bon', teks: 'buka Harga & Pemasok → Bon pemasok' }),
      ...
      baris('laporan', 'dokumen', 'Laporan', pertama ? 'catatan pertama ' + tanggalPendek(pertama) + ' · laba bersih bulan ini ' + RP(laba.labaBersih) + (laba.jumlahTanpaHpp ? ' (' + laba.jumlahTanpaHpp + ' baris tanpa modal tidak ikut)' : '') : 'belum ada catatan', umur ? umur + ' hari' : '—', 'seluruh umur buku toko ini', false, { ke: 'laporan', keluarga: 'laba', teks: 'buka Laporan' }),
    ] },
    { id: 'alat', nama: 'Alat toko', ket: 'yang dikerjakan, bukan yang dibaca', isi: [
      baris('harga', 'label', 'Katalog harga', 'harga jual per merek, karung & literan' + (K.rugi ? ' · ' + K.rugi + ' merek karung di bawah modal' : K.tipis.length ? ' · paling tipis ' + K.tipis[0].merk + ' ' + RP(K.tipis[0].untung) + '/kg' : ''), K.total + ' baris', 'karung ' + K.karung + ' · kemasan ' + K.kemasan + ' · literan ' + K.literan, K.rugi > 0, { ke: 'harga', keluarga: 'katalog', teks: 'buka Harga & Pemasok → Katalog' }),
      ...
      baris('tutupBuku', 'buku', 'Tutup buku', 'menutup tahun buku dan menulis saldo pembuka' + (era ? '' : ' · cadangan WAJIB sebelum & sesudah'), era ? 'tahun ' + era : 'belum pernah', era ? 'saldo pembuka terakhir' : 'era tutup buku masih kosong', !era && tahunLalu, { ke: 'uang', keluarga: 'buku', teks: 'buka Uang → Tutup buku' }),
    ] },
...
export function susunTanya(kini) {
  ...
  tanya('utang', 'Toko ini berutang ke siapa, berapa?', U.total ? RP(U.total) : 'nihil', U.n ? U.up.filter((x) => x.totalUtang > 0).map((x) => x.pemasok + ' ' + RP(x.totalUtang)).join(' · ') + (U.lewat.length ? '. ' + U.lewat.length + ' bon lewat tempo.' : '.') + (U.tekor ? ' Kelebihan bayar ' + RP(U.tekor) + ' — ada kedatangan yang bonnya belum masuk buku.' : '') : 'Tidak ada bon pemasok yang terbuka.', U.lewat.length > 0 || U.tekor > 0, { ke: 'harga', keluarga: 'bon', teks: 'buka Bon pemasok' }, 'truk');
...
export function susunMenurutOrang(kini) {
  ...
  out.push({ id: 'pemasok', nama: 'Pemasok', ket: pem.length ? pem.length + ' nama memegang seluruh utang toko' : 'tidak ada utang pemasok', isi: pem.map((x) => { const lw = U.lewat.filter((b) => b.pemasok === x.pemasok); const tua = Math.max(0, ...(x.bon || []).map((b) => b.umurHari || 0));
    return baris('pm-' + x.pemasok, 'truk', x.pemasok, (x.bon || []).length + ' bon terbuka' + (tua ? ' · tertua ' + tua + ' hari' : '') + (lw.length ? ' · ' + lw.length + ' lewat tempo' : '') + (x.tekor ? ' · kelebihan bayar ' + RP(x.tekor) : ''), RP(x.totalUtang), 'utang toko kepadanya', lw.length > 0 || x.tekor > 0, { ke: 'harga', keluarga: 'bon', pemasok: x.pemasok, teks: 'buka buku bon ' + x.pemasok }); }) });
  ...
  if (lunas.length) out.push({ id: 'pegawai', nama: 'Pegawai', ket: lunas.length + ' nama, kasbonnya lunas persis', isi: lunas.map((k) => baris('pg-' + k.kunci, 'orang', k.nama, 'kasbon diambil ' + RP(k.ambil) + ' dan dikembalikan ' + RP(k.bayar) + ' — tidak bersisa', 'lunas', k.mutasi.length + ' baris kasbon', false, { ke: 'uang', keluarga: 'upah', orang: k.nama, teks: 'buka Uang → Orang & upah' })) });
...
export const MN_LAYAR = [
  ...
  { nama: 'Katalog harga', sub: 'papan kapur, satu kalimat, dampak, pasar, belah, label rak · draf → terbit sekaligus', tujuan: { ke: 'harga', keluarga: 'katalog', teks: 'buka Harga & Pemasok → Katalog' } }, { nama: 'Bon pemasok', sub: 'tusukan bon, jatuh tempo, buku bon, bayar, bon lama, kartu pemasok', tujuan: { ke: 'harga', keluarga: 'bon', teks: 'buka Harga & Pemasok → Bon pemasok' } },
  { nama: 'Belanja ke pemasok', sub: 'isi truk, kapan habis, daftar belanja, kirim pesanan WhatsApp', tujuan: { ke: 'harga', keluarga: 'belanja', teks: 'buka Harga & Pemasok → Belanja' } }, // putaran 19: keluarga Laporan & Dokumen — Laba, Harian, Bulanan, Neraca, Dokumen, Setelan tidak lagi menunjuk sistem lama
  { nama: 'Laba', sub: 'margin kotor · laba bersih · diterima tunai, per bulan; susut, nota rugi, tanpa modal', tujuan: { ke: 'laporan', keluarga: 'laba', teks: 'buka Laporan → Laba' } }, { nama: 'Harian', sub: 'rekap satu hari: tunai · QRIS · bon, uang keluar, buku kas, kirim WA', tujuan: { ke: 'laporan', keluarga: 'harian', teks: 'buka Laporan → Harian' } },
  { nama: 'Mingguan', sub: 'Senin–Minggu: tujuh hari, inti minggu, banding minggu lalu', tujuan: { ke: 'laporan', keluarga: 'mingguan', teks: 'buka Laporan → Mingguan' } }, { nama: 'Tahunan', sub: 'dua belas bulan, inti tahun, banding tahun lalu, DRAF sampai tutup buku', tujuan: { ke: 'laporan', keluarga: 'tahunan', teks: 'buka Laporan → Tahunan' } },
  { nama: 'Bulanan', sub: 'enam bulan, inti bulan, rekap omzet 12 bulan, tanda dilaporkan, bukti omzet', tujuan: { ke: 'laporan', keluarga: 'bulanan', teks: 'buka Laporan → Bulanan' } }, { nama: 'Neraca', sub: 'kekayaan toko: harta vs kewajiban & modal, per tanggal', tujuan: { ke: 'laporan', keluarga: 'neraca', teks: 'buka Laporan → Neraca' } },
  { nama: 'Dokumen', sub: 'laba-rugi · neraca · arus kas berkop, paket bank, banding, setor modal, slip upah, kartu piutang, rekap bon, cetak ulang nota', tujuan: { ke: 'laporan', keluarga: 'dokumen', teks: 'buka Laporan → Dokumen' } }, { nama: 'Setelan', sub: 'kop & identitas usaha, NPWP/NIB, nomor dokumen, di mana setelan lain', tujuan: { ke: 'laporan', keluarga: 'setelan', teks: 'buka Laporan → Setelan' } },
  // putaran 18: keluarga Uang — Tutup hari, Belanja harian, Bulanan (gaji · kasbon · tagihan) tidak lagi menunjuk sistem lama
  { nama: 'Uang keluar', sub: 'pita laci: biaya toko, ambil pribadi, toko berutang ke owner, tagihan bulanan, titipan tablet', tujuan: { ke: 'uang', keluarga: 'keluar', teks: 'buka Uang → Uang keluar' } }, { nama: 'Orang & upah', sub: 'buku upah sejak terakhir dibayar, hari kerja, kasbon karyawan, slip', tujuan: { ke: 'uang', keluarga: 'upah', teks: 'buka Uang → Orang & upah' } },
  { nama: 'Owner & toko', sub: 'timbangan: modal, pinjaman owner, kasbon owner, saling lunaskan', tujuan: { ke: 'uang', keluarga: 'owner', teks: 'buka Uang → Owner & toko' } }, { nama: 'Pindah uang', sub: 'laci · brankas · rekening, amankan laci, biaya admin', tujuan: { ke: 'uang', keluarga: 'pindah', teks: 'buka Uang → Pindah uang' } },
  { nama: 'Tutup hari', sub: 'lembar tutup: hitung laci, cek QRIS, sisihkan laba, timbang cepat, amankan, rekap', tujuan: { ke: 'uang', keluarga: 'tutup', teks: 'buka Uang → Tutup hari' } }, { nama: 'Tutup buku', sub: 'berita acara tahunan: periksa, cadangan, arsip, saldo pembuka, paraf, kunci', tujuan: { ke: 'uang', keluarga: 'buku', teks: 'buka Uang → Tutup buku' } },
];
